...
# from random import
# Имортирую sys, что б задать путь до папки, откуда нужно взять токен. Что б можно было импортнуть его, что б не закидывать его в код...
# import sys
# sys.path.insert(0, 'E:/Tokens/KinoPoisk/') # PC
# sys.path.insert(0, 'C:/Tokens/KinoPoisk/') # Laptop
# from Token import token 

# """"Возникала ошибка -_- Import "Token" could not be resolvedPylancereportMissingImports, подчёркивался Token, который идёт после from, я через быстрое исправление shift+. исправлял, создаётся папка в корне .vscode и там файлик settings.json который ссылается вообще на папку "Разбор чего-нибудь" как это понимать блиН??"""
# "E:\Tokens\KinoPoisk\Token" PC
# "C:\Tokens\KinoPoisk\Token.py" Laptop
...
with open(path + f"KinoPoiskKKK{page}.html", "w", encoding="utf-8") as file:
    file.write(html)
